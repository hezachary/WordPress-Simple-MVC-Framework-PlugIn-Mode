...
class mvc_ini{
    ...
    public $aryPathSecuredList = array();
    public $aryClassDefine = array();
    public $aryImageSizeList = array();
    private $blnReqProjectSecurity = false;
    
    public function loadConfig(){
        $this->strMvcRootPath = dirname(__FILE__);
        require_once(CONFIG_PATH . self::DIR_SEP . 'config.php');
    }
    
    public function loadByPath($strLoadPath, $blnReturnPath = false, $blnOnceOnly = true){
        foreach($this->aryPathList as $key => $strPath){
            $strMvcPath = is_int($key) ? $this->strMvcRootPath . self::DIR_SEP .$strPath : $strPath ;
            $strFullPath = $strMvcPath . self::DIR_SEP .$strLoadPath;
            $this->setProjectSecurity($this->blnReqProjectSecurity, $strMvcPath);
    ...
    private function setProjectSecurity($blnProjectSecurited, $strMvcPath){
        if(!$blnProjectSecurited){
            if(!in_array($strMvcPath, $this->aryPathSecuredList)){
                $this->resetProjectSecurity($strMvcPath);
                array_push($this->aryPathSecuredList, $strMvcPath);
            }
        }
    }
